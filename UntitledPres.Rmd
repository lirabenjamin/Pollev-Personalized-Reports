...
d %>% 
  ...
  annotation_custom(rasterGrob(png::readPNG("wlearn.png"),
                               ...
                               height = unit(1,"npc")),
                               0, 100, 0, 100) +
  annotation_custom(rasterGrob(png::readPNG("Loc.png"),
                               width = unit(1,"npc"),
                               height = unit(1,"npc")),
                               69.52-5, 69.52+5, 64.66+5, 64.66-5)+
  #geom_point(size = 10,shape = "\u261F",fill = "white")+  
  theme_void()+
  coord_cartesian(xlim = c(0,100), ylim = c(0,100))
```
...
if(rs[1:6] %>% is.na %>% rowSums()!= 6){
  ...
  mutate(Level = case_when(Level == T ~ 1,
    ...
    spread(Item,Level) %>% 
    mutate(`Self-transcendence` = mean(c(selftrans1,selftrans2,selftrans3),na.rm=T)) %>% 
